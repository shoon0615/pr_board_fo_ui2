...
type FormProps<TFormValues extends FieldValues, Schema> = {
    ...
    // autoMode: 'create' | 'edit'     // 'edit' | 'modify' | 'update'
    // data?: TFormValues;
    // data?: unknown;
    data?: BaseType;
    // callback?: (callback: UseFormReset<TFormValues>) => void;
};
...
>({
    ...
    /** 최초 접속 시, defaultValues(모두 빈값) 초기화 -> reset(전달받은 데이터) 초기화 */
    type SchemaType = z.infer<typeof schema>;

    // const { defaultValues as setValues } = options;  // options?: UseFormProps<TFormValues> -> options | undefined 라 불가
    const basicValues: SchemaType = options?.defaultValues ?? {};
    ...
    const methods = useForm({ ...options, resolver: zodResolver(schema), defaultValues });

    // 최초 접속 시 
    useEffect(() => {
        // methods.trigger();         // 전체 FormField 검사(validation 체크)
        // methods.trigger("title");  // 특정 FormField 검사

        // autoMode === 'create' && methods.reset(basicValues);
        // callback && callback(methods.reset);
        try {
            // data && methods.reset(data);
            // data && methods.reset(convertDefaultValues(data));
            if (data) {
        ...
        // } catch { console.error('error') }     // error 생략 가능
        } catch (error: any) {
